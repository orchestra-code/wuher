...
describe("detectCloudflareChallenge", () => {
  ...
  it("should not flag normal pages with Cloudflare JSD passive monitoring", () => {
    const normalPageWithJsd = `<!DOCTYPE html><html><head><title>Blake Harris Law</title></head><body><h1>Personal Injury Attorney</h1><p>Full page content here...</p><script>(function(){function c(){var b=a.contentDocument||a.contentWindow.document;if(b){var d=b.createElement('script');d.innerHTML="window.__CF$cv$params={r:'abc123',t:'MTc3MjUwMDA2MA=='};var a=document.createElement('script');a.nonce='';a.src='/cdn-cgi/challenge-platform/scripts/jsd/main.js';document.getElementsByTagName('head')[0].appendChild(a);";b.getElementsByTagName('head')[0].appendChild(d)}}if(document.body){var a=document.createElement('iframe');a.height=1;a.width=1;a.style.position='absolute';a.style.top=0;a.style.left=0;a.style.border='none';a.style.visibility='hidden';document.body.appendChild(a);if('loading'!==document.readyState)c();else if(window.addEventListener)document.addEventListener('DOMContentLoaded',c)}})()</script></body></html>`;
    const result = detectCloudflareChallenge(normalPageWithJsd);
    expect(result.isCloudflareProtected).toBe(false);
  });
});
